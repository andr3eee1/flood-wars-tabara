...
#define HASH_MAXN 100000 // 100k =~ 80% din 128k, nu umplem tabela mai mult
#define MAX_HASH (128 * 1024) // 128000 de liste
#define SUM_MAX_STR_LEN 1000000 // lungimea tuturor sirurilor inserate
#define HASH_NIL 0
#define HASH_NCHAR 128
#define HASH_NALPHA 26
#define HASH_SIGMA 36

char cval[HASH_NCHAR];
...
} cell[HASH_MAXN]; // vector perechi (sir, valoare) + link la urmatoarea celula
...
int computeHash( char s[] ) {
  ...
  while ( s[i] ) // sirul se termina cu '\0'
    hash = hash * HASH_SIGMA + cval[s[i++]]; // adaugam la coada
...
// sau HASH_NIL daca s[] nu e gasit
int get_cell( char s[], int hash ) {
  int p = hash_index[hash];

  while ( p != HASH_NIL && str_cmp( &string[cell[p].first], s ) )
    p = cell[p].next;
...
int getVal( char s[] ) {
  ...
  return p == HASH_NIL ? -1 : cell[p].val;
}
...
void setVal( char s[], int val ) {
  ...
  if ( p == HASH_NIL ) { // valoare negasita in tabela, adaugam o celula
